...
public interface ApplicationRepository extends JpaRepository<MentorshipApplication, Long> {
    ...
    @Query(value = "SELECT * FROM mentorship_application WHERE is_active=true",nativeQuery = true)
    List<MentorshipApplication> findAllApplications();

    @Query(value = "select * FROM mentorship_application WHERE applicant_username =?1 AND  subject_id = ?2 AND is_active = true",nativeQuery = true)
...
/*
    ...
    */
    ...
    @Query(value = "UPDATE mentorship_application SET is_active = false WHERE applicant_username = ?1 AND subject_id = ?2 ",nativeQuery = true)
